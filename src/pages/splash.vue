<template>
  <div class="splash-page">
    ...
    <q-modal v-model="showDownloading"
      ...
      <div class="row justify-center">
        <q-progress v-if="downloadData"
                    :percentage="downloadData"
                    color="positive" />
        <q-progress v-if="!downloadData"
                    indeterminate
                    color="positive" />
...
export default {
  ...
  created () {
    ...
    ipc.on(Types.UI_ACTION_CLIENTBINARYSTATUS, (event, status, data) => {
      ...
      } else if (status === 'downloading-process') {
        let datas = data.split(',')
        let currentBytes = datas[0]
        let totalBytes = datas[1]
        this.downloadData = parseInt((currentBytes / totalBytes) * 100, 10)
      }
    })
