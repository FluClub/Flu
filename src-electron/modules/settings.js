...
class Settings {
  ...
  get indexContractAddress () {
    return '0x582c973fb578ce031649d2e49596635126107906'
  }
  ...
  get ipfsPidPath () {
    return path.join(app.getPath('home'), '.flu', 'ipfs.pid')
  }
  ...
  get chainDataPath () {
    ...
    if (this.platform === 'win') {
      dataDir = path.join(dataDir, 'AppData', 'Roaming', 'indexchain')
    } else if (this.platform === 'mac') {
      dataDir = path.join(dataDir, 'Library', 'indexchain')
    } else {
      dataDir = path.join(dataDir, '.indexchain')
    }
    if (this.isTestnet) {
  ...
  get chainDataDir () {
    return path.join(this.chainDataPath, 'indexchain', 'chaindata')
  }
