...
import IndexchainNode from './indexchainNode'
...
class NodeSync extends EventEmitter {
  constructor () {
    ...
    IndexchainNode.on('state', _.bind(this._onNodeStateChanged, this))
  }
  ...
  start () {
    ...
    this._syncPromise = Q.try(() => {
      if (!(IndexchainNode.state === IndexchainNode.STATES.CONNECTED)) {
        throw new Error('Cannot sync - IndexchainNode node not yet connected')
      }
  ...
  _onNodeStateChanged (state) {
    switch (state) { // eslint-disable-line default-case
      ...
      case IndexchainNode.STATES.STOPPING:
        log.info('Indexchain node stopping, so stop sync')

        this.stop()
        break
      // auto-sync whenever node gets connected
      case IndexchainNode.STATES.CONNECTED:
        log.info('Indexchain node connected, re-start sync')
