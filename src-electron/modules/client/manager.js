  ...
  download (clientId, options, cm) {
    ...
    return Promise.resolve()
      .then(() => {
        ...
        const localFilePath = path.join(global.__statics, Settings.platform)
        ...
        if (client.version !== Settings.defaultVersion) {
          ...
          stream.on('data', (chunk) => {
            ...
            this._logger.info(`currentBytes ==> ${alreadyDownloaded}`)
            cm._emit('downloading-process', `${alreadyDownloaded},${totalBytes}`)
          })
        }
