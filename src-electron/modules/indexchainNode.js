...
const log = logger.create('IndexchainNode')

const DEFAULT_NODE_TYPE = 'indexchain'
...
/**
 * Indexchain nodes manager.
 */
class IndexchainNode extends EventEmitter {
  constructor () {
    super()
  ...
  _killIpfs () {
    let ipfsPid = fs.readFileSync(Settings.ipfsPidPath, 'utf-8')
    ipfsPid = parseInt(ipfsPid, 10)
    try {
      process.kill(ipfsPid)
    } catch (err) {
      log.error('fail to kill ipfs, possible ipfs is already dead')
    }
  }
...
IndexchainNode.STARTING = 0

export default new IndexchainNode()
